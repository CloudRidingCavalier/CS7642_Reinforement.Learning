...
def training_NNQ_Agent(EPISODES, gamma, epsilon, learning_rate, loss_fuction, h5_name):
    ...
    for e in range(EPISODES):
        ...
        # decaying epsilon when the episode is done: linear decay
        if (agent.epsilon - float(epsilon) / float(EPISODES)) > 0.001:
            agent.epsilon = agent.epsilon - float(epsilon) / float(EPISODES)
...
###############################
#
# Using mse as loss function, gamma = 0.90, epsilon = 1.0, learning_rate = 0.001, 1000 EPISODES
#
###############################

learningRate = 0.001

training_results_mse = training_NNQ_Agent(1000, 0.90, 1.0, learningRate, "mse", "_Alpha_" + str(learningRate) + "_linearE_mse.h5")
...
training_results_mse_out.to_csv("linearE_training_results_mse_out.csv", index=False)

###############################
#
# Using mse as loss function, gamma = 0.90, epsilon = 1.0, learning_rate = 0.001, 100 trails
#
###############################

testing_results_mse = testing_NNQ_Agent(100, 0.90, 1.0, learningRate, "mse", training_results_mse[5])
...
testing_results_mse_out.to_csv("linearE_testing_results_mse_out.csv", index=False)
